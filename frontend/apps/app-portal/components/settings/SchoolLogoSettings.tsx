...
function drawCropped(image: HTMLImageElement, crop: PixelCrop, rotationDeg: number, zoom: number): HTMLCanvasElement {
  const canvas = document.createElement('canvas');
  canvas.width = OUTPUT_SIZE;
  canvas.height = OUTPUT_SIZE;
  const ctx = canvas.getContext('2d');
  if (!ctx) throw new Error('Canvas not supported in this browser');
  // Map the crop selection (in the element's stable layout coordinates) into
  // natural image coordinates, accounting for the applied zoom factor.
  const natW = image.naturalWidth;
  const natH = image.naturalHeight;
  const layoutW = image.width || natW;
  const layoutH = image.height || natH;
  const safeZoom = Math.max(zoom, 1);
  const visibleW = natW / safeZoom;
  const visibleH = natH / safeZoom;
  const offsetX = (natW - visibleW) / 2;
  const offsetY = (natH - visibleH) / 2;
  const sx = offsetX + (crop.x / layoutW) * visibleW;
  const sy = offsetY + (crop.y / layoutH) * visibleH;
  const sw = (crop.width / layoutW) * visibleW;
  const sh = (crop.height / layoutH) * visibleH;
...
export function SchoolLogoSettings() {
  ...
  const applyCrop = async () => {
    ...
    try {
      const canvas = drawCropped(img, completedCrop, rotation, zoom);
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="md:col-span-2">
                <div className="rounded-lg bg-[repeating-conic-gradient(#f3f4f6_0%_25%,#ffffff_0%_50%)] bg-[length:20px_20px] p-2">
                  <div style={{ maxHeight: '360px', overflow: 'hidden', borderRadius: '8px' }} className="mx-auto w-fit">
                    <ReactCrop
                      crop={crop}
                ...
                <div className="flex items-center gap-3 mt-3 px-1">
                  <span className="text-xs font-medium text-gray-500 shrink-0 w-20">Resize</span>
                  <input
                    type="range"
                    min={1}
                    max={3}
                  ...
                  <button
                    onClick={() => setZoom(1)}
                    disabled={zoom === 1}
                    className="text-xs text-blue-600 hover:text-blue-800 font-medium disabled:text-gray-300 disabled:hover:text-gray-300 shrink-0"
                    title="Reset to 100%"
                  >
                    Reset
                  </button>
                  <span className="text-xs text-gray-400 w-10 text-right tabular-nums">{Math.round(zoom * 100)}%</span>
                </div>
                <p className="text-[11px] text-gray-400 mt-1 px-1">
                  Zoom in to use a smaller detail of the image, then drag the circle over the area you want.
                </p>
              </div>
                    ...
                    <canvas
                      ref={(canvas) => {
                        ...
                        if (canvas && img && completedCrop) {
                          const cropped = drawCropped(img, completedCrop, rotation, zoom);
