...
export const premiumStaffRecordsApi = {
  ...
  getInstitutionalLookups: (category?: string) => api.get('/premium/staff-records/lookups', { params: category ? { category } : undefined }),
  addInstitutionalLookup: (data: { category: string; label: string; parentCode?: string }) => api.post('/premium/staff-records/lookups', data),
...
generatePdf: (data: { type: string; studentId?: string; classId?: string; termId?: string; examType?: string; templateId?: string; teacherUserId?: string }) =>
  ...
  previewTeacherAnalysis: (data: { termId?: string; examType?: string; teacherUserId?: string }) =>
    api.post('/report-engine/teacher-analysis-html', data, { timeout: 120000 }),
...
export async function bulkSaveAssessmentScores(
  ...
  for (let i = 0; i < data.scores.length; i += chunkSize) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        const res = await assessmentEngineApi.scores.bulk({
          ...data,
          scores: chunk,
        }, { timeout });
