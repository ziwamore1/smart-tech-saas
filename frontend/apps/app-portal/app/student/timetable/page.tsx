...
export default function StudentTimetablePage() {
  ...
  const { data: schoolData } = useQuery({
    queryKey: ["student-school-name", user?.schoolId],
    queryFn: async () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-1">View your class schedule and lesson times</p>
      </div>
      ...
      {timetableData && (
        <div className="mb-4 flex gap-6 text-sm bg-white border border-gray-200 rounded-lg px-4 py-3 flex-wrap">
          <div>
            <span className="text-xs text-gray-500 uppercase">Student</span>
            <p className="font-semibold">{timetableData.student?.firstName} {timetableData.student?.lastName}</p>
          </div>
          <div>
            <span className="text-xs text-gray-500 uppercase">Class</span>
            <p className="font-semibold">{timetableData.class?.name || "Not assigned"}</p>
          </div>
          <div>
            <span className="text-xs text-gray-500 uppercase">Admission No.</span>
            <p className="font-semibold">{timetableData.student?.admissionNumber || "-"}</p>
          </div>
        </div>
      )}
