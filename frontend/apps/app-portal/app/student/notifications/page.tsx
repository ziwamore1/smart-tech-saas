...
export default function StudentNotificationsPage() {
  ...
  useSchoolSocket({
    'notification:new': () => {
      queryClient.invalidateQueries({ queryKey: ['student-notifications-page'] });
      queryClient.invalidateQueries({ queryKey: ['student-notifications-totals'] });
    },
  });

  const { data, isLoading } = useQuery<{ notifications: NotificationItem[]; unread: number }>({
    queryKey: ['student-notifications-page', category],
    queryFn: async () => {
  ...
  const { data: totals } = useQuery<{ notifications: NotificationItem[] }>({
    queryKey: ['student-notifications-totals'],
    queryFn: async () => {
