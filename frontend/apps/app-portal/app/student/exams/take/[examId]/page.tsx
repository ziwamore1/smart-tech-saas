      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center justify-between mt-6">
          ...
          {currentQuestion < allQuestions.length - 1 ? (
            ...
            <button onClick={() => { if (window.confirm('Submit exam? You cannot change answers after submission.')) handleSubmit(); }} disabled={submitExamMutation.isPending} className="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium disabled:opacity-50 disabled:cursor-not-allowed">
              {submitExamMutation.isPending ? (
                <><svg className="animate-spin h-4 w-4 mr-2 inline" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>Submitting...</>
              ) : 'Submit Exam'}
            </button>
          )}
        </div>
