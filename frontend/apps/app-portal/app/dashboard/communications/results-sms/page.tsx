  ...
  return <PermissionGuard permission="communications.view" fallback={<div className="p-8 text-slate-600">You do not have permission to view Results SMS.</div>}>
    <div className="min-h-full space-y-6 pb-10">
      ...
      {tab === 'compose' && <>
        ...
        {preview && !isLoading && <>
          {preview.previewWarnings?.length > 0 && <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><div className="font-semibold">Some report-card details need attention</div><div className="mt-1">Published result rows are available, but the report-card formatter used a safe fallback for {preview.previewWarnings.length} student(s).</div></div>}
...
function MessagePreview({ recipient }: { recipient: any }) { if (!recipient) return null; return <div><div className="mb-2 flex justify-between text-xs text-slate-500"><span>Exact SMS preview</span><span>{recipient.characters} characters · {recipient.segments} segment(s) · {recipient.estimatedUnits} unit(s)</span></div><div className="whitespace-pre-wrap rounded-xl border border-slate-200 bg-white p-4 font-mono text-sm leading-6 text-slate-700">{recipient.message}</div><div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500">{recipient.result.subjects.map((s: any) => <span key={s.name} className="rounded bg-slate-100 px-2 py-1">{s.name}: {recipient.result.points != null ? `Points ${s.points ?? '-'}` : s.absent ? 'Absent' : s.mark ?? '-'} {recipient.result.points == null ? (s.grade || '') : ''}</span>)}</div></div>; }
