                ...
                <div key={exam.id} style={{ background: '#fefcf9', borderRadius: '14px', border: '1px solid #f3f4f6', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
                  <div style={{ padding: '18px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                      ...
                      <button onClick={() => document.getElementById(`answer-key-${exam.id}`)?.click()} style={{ padding: '6px 12px', background: '#f0fdf4', color: '#059669', border: 'none', borderRadius: '8px', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}><i className="fa fa-key"></i> Answer Key</button>
                      <input id={`answer-key-${exam.id}`} type="file" accept=".pdf,.docx,.doc,.txt,.json" style={{ display: 'none' }} onChange={e => { const f = e.target.files?.[0]; if (f) { const fd = new FormData(); fd.append('file', f); examApi.uploadAnswerKey(exam.id, fd).then(() => loadAll()); } }} />
                      <button onClick={() => document.getElementById(`marking-key-${exam.id}`)?.click()} style={{ padding: '6px 12px', background: '#fffbeb', color: '#d97706', border: 'none', borderRadius: '8px', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}><i className="fa fa-check-double"></i> Marking Key</button>
                      <input id={`marking-key-${exam.id}`} type="file" accept=".pdf,.docx,.doc,.txt,.json" style={{ display: 'none' }} onChange={e => { const f = e.target.files?.[0]; if (f) { const fd = new FormData(); fd.append('file', f); examApi.uploadMarkingKey(exam.id, fd).then(() => loadAll()); } }} />
                      <button onClick={() => { const enabled = !exam.autoGrade; examApi.toggleAutoGrade(exam.id, enabled).then(() => loadAll()); }} style={{ padding: '6px 12px', background: exam.autoGrade !== false ? '#f0fdf4' : '#fef2f2', color: exam.autoGrade !== false ? '#059669' : '#dc2626', border: 'none', borderRadius: '8px', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}><i className="fa fa-robot"></i> {exam.autoGrade !== false ? 'Auto On' : 'Auto Off'}</button>
