    ...
    <div style={{ padding: '24px 32px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={handleSyncAll} disabled={loadingActions['syncAll']} style={{ padding: '8px 16px', background: loadingActions['syncAll'] ? '#d1d5db' : '#ea6645', color: '#fff', border: 'none', borderRadius: 6, cursor: loadingActions['syncAll'] ? 'not-allowed' : 'pointer', fontSize: 14, fontWeight: 500 }}>
            <i className={`fas ${loadingActions['syncAll'] ? 'fa-spinner fa-spin' : 'fa-sync'}`} style={{ marginRight: 6 }}></i>{loadingActions['syncAll'] ? 'Syncing...' : 'Sync All Staff'}
          </button>
        </div>
      </div>
...
function ReturnsTabWithTemplates({ profiles, templates, submissions, templateLoadError, onRefresh }: { profiles: any[]; templates: any[]; submissions: any[]; templateLoadError: string | null; onRefresh: () => void }) {
  ...
  const handleAddColumn = async () => {
    ...
    withBtnLoading('addColumn', async () => {
      try {
        await premiumStaffRecordsApi.addColumn(tid, {
        ...
        showToast('error', err?.response?.data?.message || 'Failed to add column');
      }
    });
  };
  ...
  const DATA_TYPES = ['string', 'number', 'date', 'boolean', 'email', 'phone', 'textarea', 'select', 'currency'];

  const handleColumnTypeChange = async (columnId: string, dataType: string) => {
    try {
      await premiumStaffRecordsApi.updateColumn(columnId, { dataType });
      showToast('success', `Type changed to ${dataType}`);
      const tid = selectedTemplateIdRef.current;
      if (tid) fetchColumns(tid);
    } catch {
      showToast('error', 'Failed to update column type');
    }
  };

  const handleToggleRequired = async (column: any) => {
    withBtnLoading(`req_${column.id}`, async () => {
      try {
        await premiumStaffRecordsApi.updateColumn(column.id, { isRequired: !column.isRequired });
        const tid = selectedTemplateIdRef.current;
        if (tid) fetchColumns(tid);
      } catch {
        showToast('error', 'Failed to update column');
  ...
  const handleToggleEditable = async (column: any) => {
    withBtnLoading(`edit_${column.id}`, async () => {
      try {
        await premiumStaffRecordsApi.updateColumn(column.id, { isEditable: !(column.isEditable !== false) });
        const tid = selectedTemplateIdRef.current;
        if (tid) fetchColumns(tid);
      } catch {
        showToast('error', 'Failed to update column');
  ...
  const handleReorderColumn = async (columnId: string, direction: 'up' | 'down') => {
    const tid = selectedTemplateIdRef.current;
    if (!tid) return;
    ...
    [sorted[idx].columnOrder, sorted[swapIdx].columnOrder] = [sorted[swapIdx].columnOrder, sorted[idx].columnOrder];
    const reordered = sorted.map((c: any) => ({ id: c.id, order: c.columnOrder }));
    withBtnLoading(`reorder_${columnId}`, async () => {
      try {
        await premiumStaffRecordsApi.reorderColumns(tid, reordered);
        showToast('success', 'Column reordered');
        fetchColumns(tid);
      } catch {
              ...
              <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e8ddd0', padding: 16 }}>
                <div style={{ display: 'flex', gap: 8, marginBottom: 16, alignItems: 'center' }}>
                  ...
                  <button onClick={() => handleDeleteTemplate(editTemplate.id)} disabled={btnLoading[`del_${editTemplate.id}`]} style={{ padding: '6px 14px', background: btnLoading[`del_${editTemplate.id}`] ? '#d1d5db' : '#dc2626', color: '#fff', border: 'none', borderRadius: 6, cursor: btnLoading[`del_${editTemplate.id}`] ? 'not-allowed' : 'pointer', fontSize: 13 }}>{btnLoading[`del_${editTemplate.id}`] ? 'Deleting...' : 'Delete'}</button>
                   <button onClick={() => handleQuickCompile(editTemplate.id)} disabled={btnLoading[`compile_${editTemplate.id}`]} title="Populate this configured return from canonical HR records" style={{ padding: '6px 14px', background: btnLoading[`compile_${editTemplate.id}`] ? '#d1d5db' : '#ea6645', color: '#fff', border: 'none', borderRadius: 6, cursor: btnLoading[`compile_${editTemplate.id}`] ? 'not-allowed' : 'pointer', fontSize: 13 }}><i className={`fas ${btnLoading[`compile_${editTemplate.id}`] ? 'fa-spinner fa-spin' : 'fa-download'}`}></i> {btnLoading[`compile_${editTemplate.id}`] ? 'Loading Staff...' : 'Load Staff Records'}</button>
                      ...
                      <tbody>
                        {templateColumns.sort((a: any, b: any) => (a.columnOrder || 0) - (b.columnOrder || 0)).map((col: any, idx: number) => (
                          <tr key={col.id} style={{ borderTop: '1px solid #f3eee8' }}>
                            <td style={{ padding: '6px 10px', color: '#6b7280', fontSize: 11 }}>{idx + 1}</td>
                            ...
                            <td style={{ padding: '6px 10px' }}>
                              <select
                                defaultValue={col.dataType || 'string'}
                            ...
                            <td style={{ padding: '6px 10px' }}>
                              <button
                                ...
                                disabled={btnLoading[`req_${col.id}`]}
                                style={{ padding: '2px 8px', border: 'none', borderRadius: 4, cursor: btnLoading[`req_${col.id}`] ? 'not-allowed' : 'pointer', fontSize: 12, background: btnLoading[`req_${col.id}`] ? '#e5e7eb' : (col.isRequired ? '#dcfce7' : '#f3f4f6'), color: col.isRequired ? '#16a34a' : '#6b7280' }}
                              >{btnLoading[`req_${col.id}`] ? '...' : (col.isRequired ? 'Yes' : 'No')}</button>
                            </td>
                            <td style={{ padding: '6px 10px' }}>
                              <button
                                onClick={() => handleToggleEditable(col)}
                                disabled={btnLoading[`edit_${col.id}`]}
                                style={{ padding: '2px 8px', border: 'none', borderRadius: 4, cursor: btnLoading[`edit_${col.id}`] ? 'not-allowed' : 'pointer', fontSize: 12, background: btnLoading[`edit_${col.id}`] ? '#e5e7eb' : (col.isEditable !== false ? '#dcfce7' : '#f3f4f6'), color: col.isEditable !== false ? '#16a34a' : '#6b7280' }}
                              >{btnLoading[`edit_${col.id}`] ? '...' : (col.isEditable !== false ? 'Yes' : 'No')}</button>
                            </td>
                            <td style={{ padding: '6px 10px' }}>
                              <button
                                onClick={() => handleColumnToggle(col)}
                                disabled={btnLoading[`toggle_${col.id}`]}
                                style={{ padding: '2px 8px', border: 'none', borderRadius: 4, cursor: btnLoading[`toggle_${col.id}`] ? 'not-allowed' : 'pointer', fontSize: 12, background: btnLoading[`toggle_${col.id}`] ? '#e5e7eb' : (col.isVisible !== false ? '#dcfce7' : '#f3f4f6'), color: col.isVisible !== false ? '#16a34a' : '#6b7280' }}
                              >{btnLoading[`toggle_${col.id}`] ? '...' : (col.isVisible !== false ? 'Active' : 'Off')}</button>
                            </td>
                            <td style={{ padding: '6px 10px' }}>
                              <div style={{ display: 'flex', gap: 2 }}>
                                <button onClick={() => handleReorderColumn(col.id, 'up')} disabled={idx === 0 || !!btnLoading[`reorder_${col.id}`]} style={{ padding: '2px 5px', border: '1px solid #d1d5db', borderRadius: 4, cursor: idx === 0 || btnLoading[`reorder_${col.id}`] ? 'not-allowed' : 'pointer', fontSize: 11, background: '#fff', opacity: idx === 0 || btnLoading[`reorder_${col.id}`] ? 0.4 : 1 }}>▲</button>
                                <button onClick={() => handleReorderColumn(col.id, 'down')} disabled={idx === templateColumns.length - 1 || !!btnLoading[`reorder_${col.id}`]} style={{ padding: '2px 5px', border: '1px solid #d1d5db', borderRadius: 4, cursor: idx === templateColumns.length - 1 || btnLoading[`reorder_${col.id}`] ? 'not-allowed' : 'pointer', fontSize: 11, background: '#fff', opacity: idx === templateColumns.length - 1 || btnLoading[`reorder_${col.id}`] ? 0.4 : 1 }}>▼</button>
                ...
                {templateColumns.length > 0 && (
                  <div style={{ marginTop: 16, borderTop: '1px solid #e8ddd0', paddingTop: 16 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                      <span style={{ fontWeight: 600, fontSize: 14 }}>Template Preview</span>
                        ...
                        <button
                          onClick={() => {
                            ...
                            sorted.forEach((col: any) => {
                              if (col.dataType === 'NUMBER') newRow[col.columnName] = 0;
                              else if (col.dataType === 'DATE') newRow[col.columnName] = new Date().toISOString().slice(0, 10);
                              else if (col.dataType === 'BOOLEAN') newRow[col.columnName] = false;
                              else if (col.dataType === 'CURRENCY') newRow[col.columnName] = 0;
                      ...
                      <table className="hr-returns-table" style={{ fontSize: 12 }}>
                        <thead>
                          <tr style={{ background: '#f9f5f0', textAlign: 'left' }}>
                            ...
                            {templateColumns.sort((a: any, b: any) => (a.columnOrder || 0) - (b.columnOrder || 0)).map((col: any) => (
                              <th key={col.id} style={{ padding: '6px 8px', fontWeight: 600, whiteSpace: 'nowrap' }}>
                                {col.columnLabel || col.columnName}
                                {col.isRequired && <span style={{ color: '#dc2626', marginLeft: 2 }}>*</span>}
                                <span style={{ color: '#9ca3af', fontSize: 10, marginLeft: 4, fontWeight: 400 }}>{col.dataType}</span>
                              </th>
                            ))}
                            <th style={{ padding: '6px 8px', fontWeight: 600, width: 50 }}>Actions</th>
                          </tr>
                        </thead>
                        <tbody>
                          {sampleRows.length === 0 ? (
                            <tr>
                              <td colSpan={templateColumns.length + 2} style={{ padding: 16, textAlign: 'center', color: '#9ca3af' }}>
                                No rows yet. Click "Add Row" to add sample data for preview.
                              </td>
                            </tr>
                          ) : (
                            sampleRows.map((row: any, rIdx: number) => (
                              <tr key={rIdx} style={{ borderTop: '1px solid #f3eee8' }}>
                                <td style={{ padding: '4px 8px', color: '#6b7280', fontSize: 11 }}>{rIdx + 1}</td>
                                {templateColumns.sort((a: any, b: any) => (a.columnOrder || 0) - (b.columnOrder || 0)).map((col: any) => (
                                  <td key={col.id} style={{ padding: '4px 8px' }}>
                                    <input
                                      ...
                                      onChange={e => {
                                        ...
                                        updated[rIdx] = { ...updated[rIdx], [col.columnName]: e.target.value };
                                        setSampleRows(updated);
                                      }}
                                      placeholder={`Enter ${col.columnLabel || col.columnName}`}
                                      style={{ width: '100%', minWidth: 100, border: '1px solid #d1d5db', padding: '3px 6px', borderRadius: 4, fontSize: 12 }}
                                    />
                                  </td>
                                ))}
                                <td style={{ padding: '4px 8px' }}>
                                  <button
                                    onClick={() => setSampleRows(sampleRows.filter((_: any, i: number) => i !== rIdx))}
                                    style={{ padding: '2px 6px', background: '#fecaca', color: '#dc2626', border: 'none', borderRadius: 4, cursor: 'pointer', fontSize: 11 }}
                                  >Delete</button>
                                </td>
                              </tr>
                            ))
