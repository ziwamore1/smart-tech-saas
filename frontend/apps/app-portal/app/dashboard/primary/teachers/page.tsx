...
import { teacherApi, api, teachingAssignmentApi, classApi, subjectApi, academicYearApi, roleApi, accessApi } from '@/lib/api';
import { useAuth } from '@/lib/auth-context';
import { usePermissions } from '@/lib/permission-context';

const PRIMARY_ROLES = [
  { name: 'Head Teacher', icon: '🎓', color: '#059669' },
  { name: 'Deputy Director', icon: '⭐', color: '#7c3aed' },
...
export default function PrimaryTeachersPage() {
  ...
  const { isLoading: accessLoading } = useQuery({
    queryKey: ['primary-user-access', accessUserId],
  ...
  const saveAccessMutation = useMutation({
    ...
    onSuccess: () => { setMessage({ type: 'success', text: 'Permissions saved.' }); setShowAccessModal(false); },
  ...
  const [editForm, setEditForm] = useState({
    ...
    dateOfBirth: '', department: '', staffType: 'TEACHING', qualification: '',
    specialization: '', yearsOfExperience: '', employeeId: '', hireDate: '',
    address: '', emergencyContact: '', emergencyPhone: '', role: '',
  });
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button onClick={async () => {
                          ...
                          if (userId) {
                            try {
                              const rolesRes = await roleApi.getUserRoles(userId);
                              const rolesData = rolesRes.data?.data || rolesRes.data || [];
                              setEditCurrentRoles(Array.isArray(rolesData) ? rolesData.map((r: any) => r.roleName || r.name || r.role?.name || r).filter(Boolean) : []);
                            } catch { setEditCurrentRoles([]); }
                          } else { setEditCurrentRoles([]); }
        ...
        <div className="fixed inset-0 bg-gray-900/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex items-start justify-between mb-6">
              <div><h2 className="text-2xl font-bold text-gray-900">Manage Access</h2><p className="text-sm text-gray-500 mt-1">{accessTeacher?.user?.firstName} {accessTeacher?.user?.lastName}</p></div>
              <button onClick={() => setShowAccessModal(false)} className="text-gray-400 hover:text-gray-700 text-xl" aria-label="Close">×</button>
            </div>
            {accessLoading ? <div className="py-12 text-center text-gray-500">Loading permissions...</div> : <div className="space-y-6">
              ...
              {permissionGroups.map((group) => <section key={group.label}><h3 className="font-semibold text-gray-900 mb-3">{group.label}</h3><div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{group.values.map((permission) => <label key={permission} className="flex items-center gap-3 rounded-lg border border-gray-200 px-3 py-2 hover:bg-gray-50 cursor-pointer"><input type="checkbox" checked={accessPermissions.includes(permission)} onChange={(event) => setAccessPermissions((current) => event.target.checked ? [...current, permission] : current.filter((item) => item !== permission))} className="rounded border-gray-300 text-violet-600 focus:ring-violet-500" /><span className="text-sm text-gray-700">{permission.replaceAll('_', ' ').toLowerCase().replace(/(^| )\w/g, (letter) => letter.toUpperCase())}</span></label>)}</div></section>)}
            </div>}
            <div className="flex justify-end gap-3 pt-6 mt-6 border-t"><button onClick={() => setShowAccessModal(false)} className="px-4 py-2 border rounded-lg hover:bg-gray-50">Cancel</button><button onClick={() => saveAccessMutation.mutate()} disabled={accessLoading || saveAccessMutation.isPending || !accessUserId} className="px-4 py-2 bg-violet-600 text-white rounded-lg hover:bg-violet-700 disabled:bg-gray-400">{saveAccessMutation.isPending ? 'Saving...' : 'Save Permissions'}</button></div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex gap-3 justify-end pt-6">
              ...
              <button onClick={async () => {
                ...
                if (editForm.role) {
                  try {
                    const userId = selectedTeacher.userId || selectedTeacher.user?.id;
                    const oldRoles = editCurrentRoles.filter(r => PRIMARY_ROLES.some(ar => ar.name === r));
