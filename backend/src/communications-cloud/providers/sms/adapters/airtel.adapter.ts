...
export class AirtelAdapter implements SmsProvider {
  ...
  constructor(config: AirtelConfig) {
    ...
    this.baseUrl = (config.baseUrl || 'https://api.airtel.africa').replace(/\/+$/, '');
    ...
    try {
      ...
      if (data?.status === 'success' || data?.success || data?.code === 200) {
        return {
          success: true,
          provider: 'airtel',
          messageId: `airtel_${data?.messageId || data?.id || Date.now()}`,
      ...
      return {
        success: false,
        provider: 'airtel',
        messageId: `airtel_failed_${Date.now()}`,
        status: 'FAILED',
      ...
      return {
        success: false,
        provider: 'airtel',
        messageId: `airtel_error_${Date.now()}`,
        status: 'FAILED',
