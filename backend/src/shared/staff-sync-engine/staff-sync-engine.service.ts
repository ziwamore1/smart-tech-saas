      ...
      if (existing) {
        ...
        const profile = await this.prisma.staffHrProfile.create({
          data: {
            ...
            schoolId,
            employeeNumber: teacher.employeeNo,
            teacherName: teacher.user ? `${teacher.user.firstName || ''} ${teacher.user.lastName || ''}`.trim() : null,
            ...
            professionalQualification: teacher.qualification || null,
            specialization: teacher.specialization || null,
            employmentStatus: 'ACTIVE',
            employmentType: teacher.staffType === 'TEACHING' ? 'PERMANENT' : 'CONTRACT',
            syncHash: hash,
            syncStatus: 'SYNCED',
            lastSyncedAt: new Date(),
          },
