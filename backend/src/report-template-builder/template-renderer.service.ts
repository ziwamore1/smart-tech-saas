...
<body>
    ...
    if (sigBlock && !html.includes('{{digital_signature}}')) {
      if (html.includes('class="cert-page"')) {
        ...
        // so it never spills onto a second PDF page.
          const footer = `<div class="certificate-authenticity" style="position:fixed;left:50%;transform:translateX(-50%);bottom:88px;width:620px;max-height:24px;overflow:hidden;z-index:50;"><style>.certificate-authenticity>div{margin-top:0!important;gap:8px!important;line-height:1!important}.certificate-authenticity>div>div{min-width:0!important;padding:0 3px!important}.certificate-authenticity *{font-size:7px!important;line-height:1.05!important;margin-top:0!important;margin-bottom:0!important;padding-top:0!important;padding-bottom:0!important}.certificate-authenticity>div>div>div:first-child{font-size:7px!important}.certificate-authenticity>div>div>div:nth-child(2){font-size:8px!important;margin:1px 0!important}.certificate-authenticity>div>div>div:nth-child(3){font-size:7px!important}</style>${sigBlock}</div>`;
  ...
  private async signReportCardSignatories(
    ...
    // Signature issuance must NEVER block stamp issuance. If anything in the
    // signatory pipeline fails (unresolvable signatory table, unconfigured
    // slots, persistence errors), we degrade to stamp-only issuance instead of
    // throwing and dropping the entire authenticity block.
    try {
      ...
      // generated document still carries the digital stamp, QR, serial and
      // verification metadata. See SignatureBridgeService graceful degradation.
      this.logger.warn(
        `Report signature pipeline failed for template ${template.id} (school ${schoolId}); continuing with stamp-only issuance: ${e?.message ?? e}`,
      );
      return { html: '', signatureRecordId: null, signatures: [] };
    }
  }
    ...
    try {
      ...
      if (template.includeSignature) {
        try {
          ...
          this.logger.warn(
            `Report signature skipped for template ${templateId}: ${e?.message ?? e}`,
          );
        }
      }
