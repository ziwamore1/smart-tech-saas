...
export const INSTITUTIONAL_LOOKUP_SEEDS: { category: string; code: string; label: string; parentCode?: string; sortOrder?: number }[] = [
  ...
  ...['ECE', 'Grade 1', 'Grade 2', 'Grade 3', 'Grade 4', 'Grade 5', 'Grade 6', 'Grade 7', 'Grade 8', 'Form 2', 'Form 3', 'Form 4', 'Form 5', 'Form 6', 'Grade 10', 'Grade 11', 'Grade 12'].map((label, sortOrder) => ({ category: 'MAIN_GRADE_TAUGHT', code: label.toUpperCase().replace(/[^A-Z0-9]+/g, '_'), label, sortOrder })),
  ...['Zambian', 'Algerian', 'Angolan', 'Botswanan', 'Burundian', 'Cameroonian', 'Egyptian', 'Eswatini', 'Ethiopian', 'Ghanaian', 'Kenyan', 'Malawian', 'Mozambican', 'Namibian', 'Nigerian', 'Rwandan', 'South African', 'Tanzanian', 'Ugandan', 'Zimbabwean'].map((label, sortOrder) => ({ category: 'NATIONALITY', code: label.toUpperCase().replace(/[^A-Z0-9]+/g, '_'), label, sortOrder })),
];
