...
const ECZ_CBC_SATISFACTORY_CUT = { FORMS: 4, SECONDARY: 8 } as const;
const ECZ_CBC_BASIC_CUT = { FORMS: 3, SECONDARY: 6 } as const;
...
export function checkEczEligibility(
  ...
  const certGradeCount = resolved.filter((s) => s.points <= ECZ_CBC_SATISFACTORY_CUT[gradingSystem]).length;
  const basicGradeCount = resolved.filter((s) => s.points <= ECZ_CBC_BASIC_CUT[gradingSystem]).length;
