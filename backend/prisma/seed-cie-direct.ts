...
const targetSchoolName = 'SMART TECH SECONDARY SCHOOL';
...
async function main() {
  ...
  const schoolResult = await q(`SELECT id FROM "School" WHERE name = $1`, [targetSchoolName]);
  if (schoolResult.rows.length === 0) {
    console.log(`School "${targetSchoolName}" not found. Available schools:`);
    const allSchools = await q(`SELECT id, name FROM "School"`);
    for (const s of allSchools.rows) {
      console.log(`  ${s.name} (${s.id})`);
    }
    return;
  }
  const schoolId = schoolResult.rows[0].id;
  console.log(`Using school: ${targetSchoolName} (${schoolId})`);

  // Get existing subjects for this school
  const existing = await q(`SELECT code, id FROM "Subject" WHERE "schoolId" = $1`, [schoolId]);
    ...
    for (const eoc of subjectEocs) {
      if (!existingEocNames.has(eoc.name)) {
        await q(
          ...
          [eoc.name, subjectEocs.indexOf(eoc) + 1, subjectId, eoc.construct, schoolId]
        );
    ...
    for (const ao of subjectAos) {
      if (!existingAoNames.has(ao.name)) {
        await q(
          ...
          [ao.name, ao.weight, subjectId, schoolId]
        );
  ...
  console.log(`\nSeed complete! ${subjects.length} subjects, EoCs and AOs linked to ${targetSchoolName}.`);
}
