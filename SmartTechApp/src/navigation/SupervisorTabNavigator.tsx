...
const allDrawerScreens: DrawerScreen[] = [
  { name: 'SupervisorHome', label: 'Dashboard', icon: '📊', component: MonitoringDashboardScreen },
  { name: 'SupervisorDeptTeachers', label: 'Department Teachers', icon: '👥', component: DepartmentTeachersScreen },
...
export const SupervisorTabNavigator: React.FC = () => {
  ...
  const [activeScreen, setActiveScreen] = useState('SupervisorHome');
  ...
  useEffect(() => {
    const backHandler = BackHandler.addEventListener('hardwareBackPress', () => {
      ...
      if (activeScreen !== 'SupervisorHome') { setActiveScreen('SupervisorHome'); return true; }
      return false;
    });
  ...
  const handleNavigate = useMemo(() => (screen: string) => {
  ...
  }, [navigation, stackScreenMap, drawerScreenNames]);

  const handleLogout = () => {
    Alert.alert('Sign Out', 'Are you sure?', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Sign Out', style: 'destructive', onPress: async () => { try { await logout(); } catch (err) { console.error('Logout failed:', err); } } },
    ]);
  };
  ...
  const roleLabel = userRoles.includes('HOD') ? 'Head of Department' : 'Supervisor';
...
const styles = StyleSheet.create({
  ...
  drawerHeader: { padding: spacing.lg, borderBottomWidth: 1, borderBottomColor: colors.border, backgroundColor: colors.primary },
  ...
  avatar: { width: 64, height: 64, borderRadius: 32, backgroundColor: 'rgba(255,255,255,0.2)', justifyContent: 'center', alignItems: 'center', marginBottom: spacing.sm, borderWidth: 2, borderColor: 'rgba(255,255,255,0.4)' },
  ...
  roleBadge: { marginTop: spacing.xs, backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 12, paddingHorizontal: 12, paddingVertical: 3 },
