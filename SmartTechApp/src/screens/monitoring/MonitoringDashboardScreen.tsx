...
export const MonitoringDashboardScreen: React.FC<MonitoringProps> = ({ onToggleDrawer, onNavigate, stackNavigation }) => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'departments' | 'chain'>('overview');
    ...
    >
        ...
        <WidgetCard title="🗺️ Organisation Structure">
            ...
            {hierarchy.director && (
              <View style={styles.orgNode}>
                <View style={styles.orgBadge}>
                  <Text style={styles.orgBadgeText}>Director</Text>
                </View>
                <Text style={styles.orgName}>
                  {hierarchy.director.teacher?.user?.firstName} {hierarchy.director.teacher?.user?.lastName}
                </Text>
              </View>
            )}
              ...
              <>
                ...
                {hierarchy.deputyDirector.map((dd, i) => (
                  <View key={i} style={styles.orgNode}>
                    <View style={[styles.orgBadge, { backgroundColor: colors.warningLight }]}>
                      <Text style={[styles.orgBadgeText, { color: colors.warning }]}>Deputy Director</Text>
                    </View>
                    <Text style={styles.orgName}>
                      {dd.teacher?.user?.firstName} {dd.teacher?.user?.lastName}
                    </Text>
                  </View>
                ))}
              </>
          ...
          {hierarchy.departments?.filter(d => d.hod).map((dept, i) => (
            <TouchableOpacity
              ...
              onPress={() => stackNavigation?.navigate('DepartmentTeachers', { departmentId: dept.department.id, departmentName: dept.department.name })}
            >
      ...
      {isHod && monitoringChain && (
        <WidgetCard title="👥 My Supervision">
          <Text style={styles.supervisionHeader}>
            You supervise {supervisedTeacherCount} teacher{supervisedTeacherCount !== 1 ? 's' : ''}
          </Text>
          {monitoringChain.supervises?.map((item, i) => (
            <View key={i} style={styles.superviseeRow}>
              ...
              <View style={styles.superviseeInfo}>
                <Text style={styles.superviseeName}>
      ...
      {isHod && (
        <TouchableOpacity
          ...
          onPress={() => {
            ...
            if (myDept) {
              stackNavigation?.navigate('DepartmentTeachers', { departmentId: myDept.id, departmentName: myDept.name });
            } else {
              const allDepts = hierarchy?.departments || [];
              const myDeptFromChain = monitoringChain?.supervises?.find(s => s.department)?.department;
              if (myDeptFromChain) {
                stackNavigation?.navigate('DepartmentTeachers', { departmentId: myDeptFromChain.id, departmentName: myDeptFromChain.name });
              }
            }
          }}
        >
    ...
    >
      {departments.map((dept, i) => (
        <TouchableOpacity
          ...
          onPress={() => stackNavigation?.navigate('DepartmentTeachers', { departmentId: dept.id, departmentName: dept.name })}
        >
  ...
  const renderChainTab = () => (
    ...
    >
      <WidgetCard title="My Positions">
        ...
        {(!monitoringChain?.positions || monitoringChain.positions.length === 0) && (
          <Text style={styles.emptyText}>No positions assigned.</Text>
        )}
      </WidgetCard>
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      <View style={styles.tabBar}>
        {['overview', 'departments', 'chain'].map((tab) => (
          <TouchableOpacity
            key={tab}
          ...
          >
            <Text style={[styles.tabText, activeTab === tab && styles.activeTabText]}>
              {tab === 'overview' ? 'Overview' : tab === 'departments' ? 'Departments' : 'Chain'}
            </Text>
          </TouchableOpacity>
        ))}
      </View>
